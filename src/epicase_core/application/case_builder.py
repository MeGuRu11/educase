...
(неизменяемость, конструирование этапов) живут здесь. Обратное преобразование загруженного
``Case`` в ``CaseDraft`` для редактирования реализовано в ``application.case_loader``.
"""
...
class AssetRef:
    ...
    ``data`` — байты ассета, восстановленные из памяти при открытии кейса на правку (путь к
    файлу при загрузке утрачен); если задано, ``assets.py`` берёт их вместо чтения файла. У
    нового выбора файла в пикере остаётся ``None`` — байты читаются по ``source_path``.
    """
